...
namespace Ironforge\GameBundle\Controller;
...
class ClaimedBadgeController extends Controller
{
    ...
    {
        $claimedBadges = $this->get('ironforge.game.repository.claimed_badge')
            ->findAll();
    ...
    {
        $claimedBadge = $this->get('ironforge.game.repository.claimed_badge')
            ->find($id);
        ...
        $claimedBadgeRemover = $this->get('ironforge.game.remover.claimed_badge');
    ...
    public function acceptAction($id)
    {
        ...
        $isUnlocked = $this->get('ironforge.game.repository.unlocked_badge')
            ->findOneBy([
                'user' => $user,
                'badge' => $badge
            ]);
        ...
        $unlockedBadgeFactory = $this->get('ironforge.game.unlocked_badge.factory');
        $unlockedBadge = $unlockedBadgeFactory->create($user, $badge);

        $errors = $validator->validate($unlockedBadge);

        if (0 === count($errors)) {
            $unlockedBadgeSaver = $this->get('ironforge.game.saver.unlocked_badge');
            $claimedBadgeRemover = $this->get('ironforge.game.remover.claimed_badge');
